...
// from the virtual address
	...
	if (vpn >= pageTableSize) {
		...
		{
			printf("Fallo de la página %d\n",pageTable[vpn].virtualPage);
			printf("Número de fallo: %d\n", (stats->numPageFaults + 1));
		}
    ...
    *physAddr = pageFrame * PageSize + offset;
	...
	else if(strcmp(stats->xtraArg,"-M") == 0)//Imprimir todo el mapeo
	{
		printf("	%d", virtAddr);
	    	printf("		%d", vpn);
		printf("			%d", offset);
	...
	if(pageTable != NULL && strcmp(stats->algoritmo,"-L") == 0)//Aumentar la edad
	{
	...
	/*if(strcmp(stats->algoritmo,"-F") == 0 && pageTable[vpn].valid == false){
		stats->fifoQueue.push(vpn);
		pageTable[vpn].valid = true;
		printf("Se agrego %d a la cola \n", vpn);
	}*/
	stats->vpnRefArray.push_back(vpn);//Introducir dentro de la cadena de referencía el VPN.
